...
public class KeychainSecurityBackedTokenPairStoreIT {
    ...
    public void e2eTestStoreReadDelete() {
        StoredTokenPair tokenPair = new StoredTokenPair(SAMPLE_ACCESS_TOKEN, SAMPLE_REFRESH_TOKEN);
        String key = "KeychainTest:http://test.com:TokenPair";
        ...
        assertArrayEquals("Retrieved Access Token is different", SAMPLE_ACCESS_TOKEN, readTokenPair.getAccessToken().getValue());
        assertArrayEquals("Retrieved Refresh Token is different", SAMPLE_REFRESH_TOKEN, readTokenPair.getRefreshToken().getValue());
    ...
    @Test
    public void shouldAddEmptyAccessToken() {
        final String key = "KeychainTest:http://test.com:TokenPair";

        StoredTokenPair tokenPair = new StoredTokenPair("".toCharArray(), SAMPLE_REFRESH_TOKEN);
        ...
        assertNotNull("Token pair not found", readTokenPair);
        assertArrayEquals("Retrieved Access Token is different", "".toCharArray(), readTokenPair.getAccessToken().getValue());
        assertArrayEquals("Retrieved Refresh Token is different", SAMPLE_REFRESH_TOKEN, readTokenPair.getRefreshToken().getValue());
    }

    @Test
    public void shouldAddEmptyRefreshToken() {
        final String key = "KeychainTest:http://test.com:TokenPair";

        StoredTokenPair tokenPair = new StoredTokenPair(SAMPLE_ACCESS_TOKEN, "".toCharArray());
        boolean success = underTest.add(key, tokenPair);
        assertTrue("Storing token pair failed", success);
        ...
        assertNotNull("Token pair not found", readTokenPair);
        assertArrayEquals("Retrieved Access Token is different", SAMPLE_ACCESS_TOKEN, readTokenPair.getAccessToken().getValue());
        assertArrayEquals("Retrieved Refresh Token is different", "".toCharArray(), readTokenPair.getRefreshToken().getValue());
    }
}